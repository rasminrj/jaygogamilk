...
const Products: React.FC = () => {
  const [products, setProducts] = useLocalStorage<Product[]>('jay_goga_products', []);
    ...
    if (editingProduct) {
      setProducts(prev => prev.map(product => 
        product.id === editingProduct.id 
          ? { ...product, name: formData.name, price: parseFloat(formData.price), quantity: parseFloat(formData.quantity), unit: formData.unit, photo: formData.photo }
          : product
      ));
    } else {
              ...
              >
                <div className="flex items-center space-x-4">
                  <div className="w-16 h-16 bg-dairy-100 rounded-lg flex items-center justify-center flex-shrink-0">
                    ...
                    <Package className={`text-dairy-600 ${product.photo ? 'hidden' : ''}`} size={24} />
                  </div>
